...
class _ChatScreenState extends State<ChatScreen> {
    ...
    try {
      ...
      if (userQuery.docs.isNotEmpty) {
        ...
        final initials = contactName.isNotEmpty
            ? contactName.trim().split(' ').map((e) => e[0]).take(2).join().toUpperCase()
            : 'P';
        if (mounted) {
          setState(() {
            _otherUserInitials = initials;
  ...
  @override
  void dispose() {    _inputController.removeListener(_onTextChanged);
  ...
  void _scrollToBottom() {
    ...
    _playerPositionSubscription = _audioPlayer.onPositionChanged.listen((Duration p) {
    ...
    _playerDurationSubscription = _audioPlayer.onDurationChanged.listen((Duration d) {
      if (mounted && _playingVoiceId != null) {
        setState(() {
          _voiceDuration = d;
        });
      ...
      if (await _audioRecorder.hasPermission()) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Microphone permission is required to record voice memos.'),
  ...
  Future<Map<String, dynamic>?> _findPendingMeeting() async {
    ...
    try {
      ...
      for (final doc in meetingsQuery.docs) {
        final data = doc.data();
        if (data['status'] == 'pending' && data['participants'].contains(otherUid)) {
  ...
  Future<void> _handleMeetingActionFromChat(
    ...
    try {
      ...
      final participants = List<String>.from(chatDoc.data()?['participants'] ?? []);
      final otherUid = participants.firstWhere((p) => p != currentUid, orElse: () => '');
      if (otherUid.isEmpty) return;      if (isAccept) {
        final meetingDoc = await FirebaseFirestore.instance.collection('meetings').doc(meetingId).get();
        ...
        if (scheduledTimestamp != null) {
          ...
          final otherConflict = await MeetingService().hasMeetingConflict(otherUid, scheduledAt);
          if (otherConflict) {
            if (!mounted) return;
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
        ...
        final responseMsg = "✅ Meeting Accepted: I'm down to meet at $location on $timeStr";
        await ChatService().sendTextMessage(chatId: _chatId!, text: responseMsg);
        ...
        await _showCancellationReasonDialog(meetingId, currentUid, otherUid, location, timeStr);
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildDateHeader(DateTime date) {
    ...
    if (_isSameDay(date, now)) {
      ...
      text = "${date.day} ${const ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][date.month - 1]} ${date.year}";
    }
  ...
  Widget build(BuildContext context) {
    if (_selectedContactName == null) {
          return Scaffold(
            backgroundColor: const Color(0xFFFAF7F5),            appBar: AppBar(
              backgroundColor: const Color(0xFFFAF7F5),
              elevation: 0,
              leading: IconButton(
              ...
              actions: [
                IconButton(
                  icon: const Icon(Icons.group_add_outlined, color: Color(0xFF7A432D)),
                ...
                if (chatDocs.isEmpty) {
                  return Center(
                    child: Column(
                      ...
                      children: [
                        const Icon(Icons.chat_bubble_outline_rounded, color: Color(0xFF8C736B), size: 48),
                        const SizedBox(height: 16),
                        const Text(
                ...
                return ListView.separated(
                  ...
                  separatorBuilder: (context, index) => const Divider(
                    ...
                    if (isGroup) {
                      ...
                      final String senderName = lastMsgInfo?['senderName'] ?? '';
                      final String displayText = senderName.isNotEmpty ? '$senderName: $lastText' : lastText;
                      ...
                      final String initials = groupName.isNotEmpty
                          ? groupName.trim().split(' ').map((e) => e[0]).take(2).join().toUpperCase()
                          : 'GP';
                      ...
                      return InkWell(
                        onTap: () {
                          setState(() {
                            ...
                            _chatId = chatDoc.id;
                            _chatDocStream = FirebaseFirestore.instance.collection('chats').doc(chatDoc.id).snapshots();
                          });
                                  ...
                                  children: [
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          ...
                                          style: TextStyle(
                                            fontFamily: 'PlusJakartaSans',
                                            fontSize: 10,
                                            color: isUnread ? const Color(0xFF7A432D) : const Color(0xFF8C736B),
                                    ...
                                    Text(
                                      ...
                                      style: TextStyle(
                                        fontFamily: 'PlusJakartaSans',
                                        fontSize: 12.5,
                                        color: isUnread ? const Color(0xFF3E1F11) : const Color(0xFF5C473E),
                                        fontWeight: isUnread ? FontWeight.bold : FontWeight.normal,
                                      ),
                                    ),
                                  ],
  ...
  return Scaffold(
          ...
          appBar: AppBar(
            ...
            title: _showSearch
                ...
                : StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                    ...
                    builder: (context, chatSnap) {
                      ...
                      if (isGroup) {
                        final groupName = chatData?['groupName'] ?? _selectedContactName ?? 'Group';
            ...
            actions: [
              if (_chatId != null)
                StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, chatSnap) {
                    ...
                    if (isGroup) {
                      return IconButton(
                        icon: const Icon(Icons.person_add_alt_1, color: Color(0xFF7A432D)),
                        onPressed: () => _showAddMemberDialogFromAppBar(context, chatData!),
                        tooltip: 'Add Member',
                      );
                    }
                    return const SizedBox.shrink();
              ...
              IconButton(
                icon: Icon(_showSearch ? Icons.close : Icons.search, color: const Color(0xFF7A432D)),
                onPressed: () {
                  setState(() {
          ...
          body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            stream: _chatId != null ? ChatService().streamMessages(_chatId!) : const Stream.empty(),
            builder: (context, snapshot) {
              ...
              final List<Message> filteredMsgs = messageDocs.map((doc) {
                ...
                final from = (senderId == currentUid) ? MessageSender.me : MessageSender.them;
                ...
                return Message(
                  ...
                  reactionsMap: Map<String, String>.from(data['reactionsMap'] ?? {}),
                  senderId: senderId,
                  ...
                  replyTo: data['replyTo'] != null ? Map<String, dynamic>.from(data['replyTo']) : null,
                  mentions: data['mentions'] != null ? List<String>.from(data['mentions']) : const [],
                  isRead: (data['readBy'] as List?) != null && (data['readBy'] as List).length > 1,
                  createdAt: dt,
                );
              }).toList();
              ...
              final listToShow = query.isEmpty
                  ? filteredMsgs
                  : filteredMsgs.where((m) => (m.text ?? '').toLowerCase().contains(query)).toList();
              ...
              return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                ...
                builder: (context, chatSnapshot) {
                  ...
                  final pinnedList = List<Map<String, dynamic>>.from(
                    (chatData?['pinnedMessages'] as List?)?.map((e) => Map<String, dynamic>.from(e)) ?? []
                  );
                  ...
                  return Column(
                    children: [                      if (pinnedList.isNotEmpty) _buildPinnedHeader(pinnedList),
                      ...
                      Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final showDateHeader = index == 0 || !_isSameDay(listToShow[index - 1].createdAt, msg.createdAt);
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (msg.replyTo != null) _buildReplyPreviewHeader(msg.replyTo!, textColor),
                    ...
                    Builder(
                      builder: (widgetContext) {
                        return GestureDetector(
                          ...
                          onTapDown: (details) {
                            if (isPlaying) {
                              final box = widgetContext.findRenderObject() as RenderBox?;
                              if (box != null) {
                                final width = box.size.width;
                                final localX = details.localPosition.dx;
                                final pct = (localX / width).clamp(0.0, 1.0);
                                final seekTarget = Duration(milliseconds: (duration * 1000 * pct).toInt());
                          ...
                          child: Row(
                            ...
                            children: List.generate(24, (i) {
                              ...
                              final double h = (i % 5 == 0) ? 16 : (i % 3 == 0) ? 12 : (i % 2 == 0 ? 8 : 4);
                              final isActive = isPlaying && (i / 24.0) <= _voiceProgress;
                              return Container(
                                width: 2.2,
                                height: h,
                                decoration: BoxDecoration(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      msg.fileSize != null ? '${(msg.fileSize! / 1024 / 1024).toStringAsFixed(1)} MB' : '1.2 MB',
                      style: TextStyle(
                        fontFamily: 'PlusJakartaSans',
                        fontSize: 10,
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      final responseMsg = "❌ Meeting Cancelled: I won't be able to meet at $location on $timeStr. Reason: $finalReason";
                      await ChatService().sendTextMessage(chatId: _chatId!, text: responseMsg);
            ...
            child: Column(
              ...
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'URL (required)'),
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await ChatService().sendLinkMessage(
                  chatId: _chatId!,
                  url: url,
                  title: titleController.text.trim().isNotEmpty ? titleController.text.trim() : 'Useful Link',
                  description: descController.text.trim().isNotEmpty ? descController.text.trim() : 'Shared via BoardingPass Chat',
                );
              },
  ...
  void _showCreateGroupModal(BuildContext context) async {
    ...
    final imageController = TextEditingController(
      text: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?w=300&auto=format&fit=crop&q=80'
    );
                    ...
                    children: [
                        ...
                        const Padding(
                          ...
                          child: Center(
                            child: Text(
                              ...
                              style: TextStyle(color: Color(0xFF8C736B), fontSize: 13),
                            ),
                          ),
                        )
                      else
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          try {
                            final newChatId = await ChatService().createGroupChat(
                              groupName: name,
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return ListTile(
                        ...
                        title: Text(u.name, style: const TextStyle(fontFamily: 'PlusJakartaSans', fontWeight: FontWeight.bold)),
                  ...
                  child: ListView(
                    ...
                    children: [
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            title: Text(
                              ...
                              style: const TextStyle(fontFamily: 'PlusJakartaSans', fontSize: 13, fontWeight: FontWeight.bold),
                            ),
                            subtitle: Text(
                              isMemberAdmin ? 'Group Admin' : 'Member',
                              style: const TextStyle(fontSize: 11, color: Color(0xFF8C736B)),
                            ),
                            trailing: (isAdmin && !isMe)
                                ? PopupMenuButton<String>(
                                    onSelected: (action) async {
                                      if (action == 'remove') {
                                        final newList = List<String>.from(participants)..remove(member.uid);
                                        final newAdmins = List<String>.from(admins)..remove(member.uid);
                                        await ChatService().updateGroupSettings(
                                          chatId: _chatId!,
                                        ...
                                        final newAdmins = [...admins, member.uid];
                                        await ChatService().updateGroupSettings(
                                          chatId: _chatId!,
                                          admins: newAdmins,
                                        );
                                    ...
                                    itemBuilder: (context) => [
                                      if (!isMemberAdmin)
                                        const PopupMenuItem(value: 'make_admin', child: Text('Promote Admin')),
                                      if (isMemberAdmin)
                                        const PopupMenuItem(value: 'remove_admin', child: Text('Revoke Admin')),
                      ...
                      ElevatedButton(
                        ...
                        onPressed: () async {
                          // Leave Group flow
                          final newList = List<String>.from(participants)..remove(currentUid);
                          final newAdmins = List<String>.from(admins)..remove(currentUid);
                          ...
                          if (newList.isEmpty) {
                            ...
                            final nextAdmins = newAdmins.isEmpty ? [newList.first] : newAdmins;
                            await ChatService().updateGroupSettings(
                              chatId: _chatId!,
                              participants: newList,
                              admins: nextAdmins,
  ...
  Widget _buildPinnedHeader(List<Map<String, dynamic>> pinnedList) {
    ...
    return Container(
      decoration: const BoxDecoration(
        color: Color(0xFFFAF7F5),
        border: Border(bottom: BorderSide(color: Color(0xFFE8E2DD), width: 1.2)),
      ),
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
      child: Row(
        children: [
          const Icon(Icons.push_pin_rounded, size: 16, color: Color(0xFF7A432D)),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
          ...
          IconButton(
            ...
            onPressed: () async {
              if (_chatId != null) {
                await ChatService().unpinMessage(chatId: _chatId!, messageId: lastPin['id']);
              }
            },
          ),
        ],
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: ['👍', '❤️', '😂', '😮', '😢', '🙏'].map((emoji) {
                    final isReacted = msg.reactionsMap[FirebaseAuth.instance.currentUser?.uid] == emoji;
                    return GestureDetector(
                      onTap: () async {
                        Navigator.pop(context);
                        if (_chatId != null) {
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          color: isReacted ? const Color(0xFFE5A475).withValues(alpha: 0.2) : Colors.transparent,
                        ),
              ...
              ListTile(
                ...
                title: const Text('Reply', style: TextStyle(fontFamily: 'PlusJakartaSans')),
                onTap: () {
                  Navigator.pop(context);
                  setState(() {
                    _selectedReplyMsg = {
              ...
              ListTile(
                leading: const Icon(Icons.push_pin_outlined, color: Color(0xFF7A432D)),
                title: const Text('Pin Message', style: TextStyle(fontFamily: 'PlusJakartaSans')),
                onTap: () async {
                  Navigator.pop(context);
                  if (_chatId != null) {
  ...
  Widget _buildReplyPreviewHeader(Map<String, dynamic> replyData, Color textColor) {
    return Container(
      margin: const EdgeInsets.only(bottom: 6),
      padding: const EdgeInsets.all(8),
      decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: counts.entries.map((e) {
          return Padding(
            ...
            child: Text(
              '${e.key} ${e.value}',
              style: const TextStyle(fontSize: 9, fontFamily: 'PlusJakartaSans'),
            ),
          );
        }).toList(),
      ),
              ...
              children: List.generate(3, (index) {
                return AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    ...
                    double value = math.sin((_controller.value * 2 * math.pi) - delay);
                    value = (value + 1) / 2;
                    return Container(
                      ...
                      height: 5,
                      decoration: BoxDecoration(
                        color: const Color(0xFF7A432D).withValues(alpha: 0.3 + 0.7 * value),
                        shape: BoxShape.circle,
                      ),
                    );
                  },
